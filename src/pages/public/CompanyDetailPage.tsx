...
export function CompanyDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadCompany = async () => {
      ...
      try {
        ...
        if (active) {
          setError(
            'Không tìm thấy thông tin công ty hoặc không thể tải dữ liệu.',
          )
        }
      } finally {
        if (active) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const savedReview = ownReview
        ? await reviewService.updateOwned(ownReview.id, user.id, {
            rating,
            title,
            content,
          })
    ...
    try {
      if (ownFollow) {
        ...
        setCompanyFollows((current) =>
          current.filter((follow) => follow.id !== ownFollow.id),
        )
        ...
        const createdFollow = await companyFollowService.create(
          user.id,
          company.id,
        )
  ...
  const backPath =
    requestedBackPath && /^\/jobs\/[^/]+$/.test(requestedBackPath)
      ? requestedBackPath
      : '/companies'
      ...
      <section className="mt-5 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="space-y-3 sm:text-right">
              ...
              {user?.role === 'candidate' ? (
                <button
                  ...
                  onClick={() => void toggleCompanyFollow()}
                  type="button"
                >
                  {togglingFollow
                    ? 'Đang cập nhật...'
        ...
        <div className="grid gap-7 p-6 sm:p-9 lg:grid-cols-[1fr_300px]">
          ...
          <dl className="space-y-4 rounded-2xl bg-slate-50 p-5 text-sm">
            ...
            <div>
              <dt className="text-slate-400">Quy mô</dt>
              <dd className="mt-1 font-semibold text-slate-700">
            ...
            <div>
              <dt className="text-slate-400">Việc làm đang mở</dt>
              <dd className="mt-1 font-semibold text-slate-700">
                    ...
                    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                      <div className="flex gap-3">
                        <div className="grid size-10 shrink-0 place-items-center rounded-full bg-slate-100 font-black text-slate-600">
                          {(
                            candidateNames.get(review.candidateId) ??
                            'Ứng viên'
                          ).charAt(0)}
                        </div>
