...
async function getOwnedReview(id: string, candidateId: string): Promise<Review> {
  const review = await apiClient.get<Review>(`/reviews/${encodeURIComponent(id)}`)
  if (review.candidateId !== candidateId) {
    throw new Error('Bạn không có quyền chỉnh sửa hoặc xóa đánh giá này.')
  }
  return review
}

async function validateParticipants(candidateId: string, employerId: string) {
  const [candidate, employer] = await Promise.all([
    apiClient.get<User>(`/users/${encodeURIComponent(candidateId)}`),
    apiClient.get<User>(`/users/${encodeURIComponent(employerId)}`),
  ])
  if (candidate.role !== 'candidate') throw new Error('Chỉ ứng viên mới có thể gửi đánh giá.')
  if (employer.role !== 'employer' || !employer.companyProfile) {
    throw new Error('Công ty được đánh giá không hợp lệ.')
  }
}
...
export const reviewService = {
  ...
  getByEmployer: (employerId: string) =>
    apiClient.get<Review[]>(`/reviews?employerId=${encodeURIComponent(employerId)}`),

  getByCandidate: (candidateId: string) =>
    apiClient.get<Review[]>(`/reviews?candidateId=${encodeURIComponent(candidateId)}`),
  ...
  async create(payload: CreateReviewPayload): Promise<Review> {
    if (!payload.candidateId.trim()) throw new Error('Không xác định được ứng viên đánh giá.')
    if (!payload.employerId.trim()) throw new Error('Không xác định được công ty được đánh giá.')
    validateRating(payload.rating)
    const normalizedText = normalizeText(payload.title, payload.content)

    await validateParticipants(payload.candidateId, payload.employerId)
    ...
    if (duplicate.length > 0) {
      throw new Error('Bạn đã đánh giá công ty này. Hãy sửa đánh giá hiện có thay vì tạo mới.')
    }
  ...
  async updateOwned(
    ...
    const review = await getOwnedReview(id, candidateId)
    const rating = payload.rating ?? review.rating
    ...
    return apiClient.patch<Review>(`/reviews/${encodeURIComponent(id)}`, {
      rating,
      ...normalizedText,
  ...
  async removeOwned(id: string, candidateId: string): Promise<void> {
    await getOwnedReview(id, candidateId)
