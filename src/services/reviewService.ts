...
function normalizeText(title: string, content: string) {
  ...
  if (!normalizedTitle) {
    throw new Error('Tiêu đề đánh giá không được để trống.')
  }
...
async function validateParticipants(
  ...
  if (candidate.role !== 'candidate') {
    throw new Error('Chỉ ứng viên mới có thể gửi đánh giá.')
  }
...
export const reviewService = {
  ...
  async create(payload: CreateReviewPayload): Promise<Review> {
    if (!payload.candidateId.trim()) {
      throw new Error('Không xác định được ứng viên đánh giá.')
    }

    if (!payload.employerId.trim()) {
      throw new Error('Không xác định được công ty được đánh giá.')
    }
